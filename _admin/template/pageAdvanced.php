...
<?php include tpl('menu');?>
	<div class="main">
		<h2 class="section-header">自定义页面(<?php _e($page['name'])?>) - <a href="./?m=page&a=edit&key=<?php _e($key)?>">基本</a>
					- 高级
		...
		<?php
			w('select')->set('name','COOKIES设置')
					...
					->e();
			w('textarea')->set('name','自定义COOKIES')
					->set('key','diyCookies')
					->set('value',$page['diyCookies'])
		...
		<?php
			w('select')->set('name','伪造ip')
					...
					->e();
			w('textarea')->set('name','自定义ip')
					->set('key','diyIp')
					->set('value',$page['diyIp'])
